...
template <int align>
static inline void gmx_simdcall gatherLoadTranspose(
    const float *base, const std::int32_t offset[], SimdFloat *v0,
    SimdFloat *v1, SimdFloat *v2, SimdFloat *v3) {
  ...
  auto r0 = v0->simdInternal_.native_register();
  auto r1 = v1->simdInternal_.native_register();
  auto r2 = v2->simdInternal_.native_register();
  auto r3 = v3->simdInternal_.native_register();
  _MM_TRANSPOSE4_PS(r0, r1, r2, r3);
  // _MM_TRANSPOSE4_PS(v0->simdInternal_.native_register(),
  // v0->simdInternal_.native_register(), v2->simdInternal_.native_register(),
  // v3->simdInternal_.native_register());
  v0->simdInternal_ = nsimd::pack<float>(r0);
  v1->simdInternal_ = nsimd::pack<float>(r1);
  v2->simdInternal_ = nsimd::pack<float>(r2);
  v3->simdInternal_ = nsimd::pack<float>(r3);
}
...
gatherLoadTranspose(const float *base, const std::int32_t offset[],
  ...
  v0->simdInternal_ = _mm_castpd_ps(
      _mm_load_sd(reinterpret_cast<const double *>(base + align * offset[0])));
  v1->simdInternal_ = _mm_castpd_ps(
      _mm_load_sd(reinterpret_cast<const double *>(base + align * offset[1])));
  t1 = _mm_castpd_ps(
      _mm_load_sd(reinterpret_cast<const double *>(base + align * offset[2])));
  t2 = _mm_castpd_ps(
      _mm_load_sd(reinterpret_cast<const double *>(base + align * offset[3])));
...
gatherLoadUTranspose(const float *base, const std::int32_t offset[],
                     SimdFloat *v0, SimdFloat *v1, SimdFloat *v2) {
...
transposeScatterStoreU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  // general case, not aligned to 4-byte boundary
  t1 = _mm_unpacklo_ps(v0.simdInternal_.native_register(),
                       v1.simdInternal_.native_register());
  t2 = _mm_unpackhi_ps(v0.simdInternal_.native_register(),
                       v1.simdInternal_.native_register());
  _mm_storel_pi(reinterpret_cast<__m64 *>(base + align * offset[0]), t1);
  _mm_store_ss(base + align * offset[0] + 2,
  ...
  _mm_store_ss(base + align * offset[1] + 2,
               _mm_shuffle_ps(v2.simdInternal_.native_register(),
                              v2.simdInternal_.native_register(),
                              _MM_SHUFFLE(1, 1, 1, 1)));
...
transposeScatterIncrU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  if (align < 4) {
    t5 = _mm_unpacklo_ps(v1.simdInternal_.native_register(),
                         v2.simdInternal_.native_register());
    t6 = _mm_unpackhi_ps(v1.simdInternal_.native_register(),
                         v2.simdInternal_.native_register());
    t7 = _mm_shuffle_ps(v0.simdInternal_.native_register(), t5,
                        _MM_SHUFFLE(1, 0, 0, 0));
    t8 = _mm_shuffle_ps(v0.simdInternal_.native_register(), t5,
                        _MM_SHUFFLE(3, 2, 0, 1));
    t9 = _mm_shuffle_ps(v0.simdInternal_.native_register(), t6,
                        _MM_SHUFFLE(1, 0, 0, 2));
    t10 = _mm_shuffle_ps(v0.simdInternal_.native_register(), t6,
                         _MM_SHUFFLE(3, 2, 0, 3));

    t1 = _mm_load_ss(base + align * offset[0]);
    t1 = _mm_loadh_pi(t1,
                      reinterpret_cast<__m64 *>(base + align * offset[0] + 1));
    t1 = _mm_add_ps(t1, t7);
    ...
    t2 = _mm_loadh_pi(t2,
                      reinterpret_cast<__m64 *>(base + align * offset[1] + 1));
    t2 = _mm_add_ps(t2, t8);
    ...
    t3 = _mm_loadh_pi(t3,
                      reinterpret_cast<__m64 *>(base + align * offset[2] + 1));
    t3 = _mm_add_ps(t3, t9);
    ...
    t4 = _mm_loadh_pi(t4,
                      reinterpret_cast<__m64 *>(base + align * offset[3] + 1));
    ...
    t1 = _mm_unpacklo_ps(v0.simdInternal_.native_register(),
                         v2.simdInternal_.native_register()); // x0 z0 x1 z1
    t2 = _mm_unpackhi_ps(v0.simdInternal_.native_register(),
                         v2.simdInternal_.native_register()); // x2 z2 x3 z3
    t3 = _mm_unpacklo_ps(v1.simdInternal_.native_register(),
                         _mm_setzero_ps()); // y0  0 y1  0
    t4 = _mm_unpackhi_ps(v1.simdInternal_.native_register(),
    ...
    if (align % 4 == 0) {
      ...
      _mm_store_ps(base + align * offset[0],
                   _mm_add_ps(_mm_load_ps(base + align * offset[0]), t5));
      _mm_store_ps(base + align * offset[1],
                   _mm_add_ps(_mm_load_ps(base + align * offset[1]), t6));
      _mm_store_ps(base + align * offset[2],
                   _mm_add_ps(_mm_load_ps(base + align * offset[2]), t7));
      _mm_store_ps(base + align * offset[3],
                   _mm_add_ps(_mm_load_ps(base + align * offset[3]), t8));
    } else {
...
transposeScatterDecrU(float *base, const std::int32_t offset[], SimdFloat v0,
                      SimdFloat v1, SimdFloat v2) {
  // This implementation is identical to the increment version, apart from
  ...
  if (align < 4) {
    ...
    if (align % 4 == 0) {
      // alignment is a multiple of 4
      _mm_store_ps(base + align * offset[0],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[0]), t5));
      _mm_store_ps(base + align * offset[1],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[1]), t6));
      _mm_store_ps(base + align * offset[2],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[2]), t7));
      _mm_store_ps(base + align * offset[3],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[3]), t8));
    } else {
      // alignment >=5, but not a multiple of 4
      _mm_storeu_ps(base + align * offset[0],
...
static inline void gmx_simdcall expandScalarsToTriplets(SimdFloat scalar,
                                                        ...
                                                        SimdFloat *triplets1,
                                                        SimdFloat *triplets2) {
  triplets0->simdInternal_ = _mm_shuffle_ps(
      scalar.simdInternal_.native_register(),
      scalar.simdInternal_.native_register(), _MM_SHUFFLE(1, 0, 0, 0));
  triplets1->simdInternal_ = _mm_shuffle_ps(
      scalar.simdInternal_.native_register(),
      scalar.simdInternal_.native_register(), _MM_SHUFFLE(2, 2, 1, 1));
  triplets2->simdInternal_ = _mm_shuffle_ps(
      scalar.simdInternal_.native_register(),
      scalar.simdInternal_.native_register(), _MM_SHUFFLE(3, 3, 3, 2));
}

template <int align>
static inline void gmx_simdcall gatherLoadBySimdIntTranspose(
    const float *base, SimdFInt32 offset, SimdFloat *v0, SimdFloat *v1,
    SimdFloat *v2, SimdFloat *v3) {
...
static inline float gmx_simdcall reduceIncr4ReturnSum(float *m, SimdFloat v0,
  ...
  r2 = _mm_add_ps(r2, r3);
  r0 = _mm_add_ps(r0, r2);
  r2 = _mm_add_ps(r0, _mm_load_ps(m));

  assert(std::size_t(m) % 16 == 0);
  _mm_store_ps(m, r2);

  __m128 b = _mm_add_ps(r0, _mm_shuffle_ps(r0, r0, _MM_SHUFFLE(1, 0, 3, 2)));
 ...
 */
static inline void gmx_simdcall avx256Transpose3By4InLanes(__m256 *v0,
                                                           __m256 *v1,
                                                           __m256 *v2,
                                                           __m256 *v3) {
...
static inline void gmx_simdcall
avx256Transpose3By4InLanes(nsimd::pack<float> *v0, nsimd::pack<float> *v1,
                           nsimd::pack<float> *v2, __m256 *v3) {
  __m256 tmp0 = v0->native_register();
  __m256 tmp1 = v1->native_register();
  __m256 tmp2 = v2->native_register();
  ...
  *v0 = nsimd::pack<float>(tmp0);
  *v1 = nsimd::pack<float>(tmp1);
  *v2 = nsimd::pack<float>(tmp2);
}
...
static inline void gmx_simdcall gatherLoadTranspose(
  ...
  __m256 r0 = _mm256_insertf128_ps(_mm256_castps128_ps256(t1), t5, 0x1);
  __m256 r1 = _mm256_insertf128_ps(_mm256_castps128_ps256(t2), t6, 0x1);
  __m256 r2 = _mm256_insertf128_ps(_mm256_castps128_ps256(t3), t7, 0x1);
  __m256 r3 = _mm256_insertf128_ps(_mm256_castps128_ps256(t4), t8, 0x1);

  tA = _mm256_unpacklo_ps(r0, r1);
  tB = _mm256_unpacklo_ps(r2, r3);
  tC = _mm256_unpackhi_ps(r0, r1);
  tD = _mm256_unpackhi_ps(r2, r3);

  v0->simdInternal_ =
      nsimd::pack<float>(_mm256_shuffle_ps(tA, tB, _MM_SHUFFLE(1, 0, 1, 0)));
  v1->simdInternal_ =
      nsimd::pack<float>(_mm256_shuffle_ps(tA, tB, _MM_SHUFFLE(3, 2, 3, 2)));
  v2->simdInternal_ =
      nsimd::pack<float>(_mm256_shuffle_ps(tC, tD, _MM_SHUFFLE(1, 0, 1, 0)));
  v3->simdInternal_ =
      nsimd::pack<float>(_mm256_shuffle_ps(tC, tD, _MM_SHUFFLE(3, 2, 3, 2)));
}
...
gatherLoadTranspose(const float *base, const std::int32_t offset[],
  ...
  t1 = _mm_loadl_pi(_mm_setzero_ps(),
                    reinterpret_cast<const __m64 *>(base + align * offset[0]));
  t2 = _mm_loadl_pi(_mm_setzero_ps(),
                    reinterpret_cast<const __m64 *>(base + align * offset[1]));
  t3 = _mm_loadl_pi(_mm_setzero_ps(),
                    reinterpret_cast<const __m64 *>(base + align * offset[2]));
  t4 = _mm_loadl_pi(_mm_setzero_ps(),
                    reinterpret_cast<const __m64 *>(base + align * offset[3]));
  t5 = _mm_loadl_pi(_mm_setzero_ps(),
                    reinterpret_cast<const __m64 *>(base + align * offset[4]));
  t6 = _mm_loadl_pi(_mm_setzero_ps(),
                    reinterpret_cast<const __m64 *>(base + align * offset[5]));
  t7 = _mm_loadl_pi(_mm_setzero_ps(),
                    reinterpret_cast<const __m64 *>(base + align * offset[6]));
  t8 = _mm_loadl_pi(_mm_setzero_ps(),
                    reinterpret_cast<const __m64 *>(base + align * offset[7]));
  ...
  v0->simdInternal_ = nsimd::pack<float>(_mm256_unpacklo_ps(tA, tB));
  v1->simdInternal_ = nsimd::pack<float>(_mm256_unpackhi_ps(tA, tB));
}
...
gatherLoadUTranspose(const float *base, const std::int32_t offset[],
  ...
  if (align % 4 == 0) {
    ...
    assert(std::size_t(base) % 16 == 0);
    t1 = _mm256_insertf128_ps(
        _mm256_castps128_ps256(_mm_load_ps(base + align * offset[0])),
        _mm_load_ps(base + align * offset[4]), 0x1);
    t2 = _mm256_insertf128_ps(
        _mm256_castps128_ps256(_mm_load_ps(base + align * offset[1])),
        _mm_load_ps(base + align * offset[5]), 0x1);
    t3 = _mm256_insertf128_ps(
        _mm256_castps128_ps256(_mm_load_ps(base + align * offset[2])),
        _mm_load_ps(base + align * offset[6]), 0x1);
    t4 = _mm256_insertf128_ps(
        _mm256_castps128_ps256(_mm_load_ps(base + align * offset[3])),
        _mm_load_ps(base + align * offset[7]), 0x1);
  } else {
    // Use unaligned loads
    t1 = _mm256_insertf128_ps(
        _mm256_castps128_ps256(_mm_loadu_ps(base + align * offset[0])),
        _mm_loadu_ps(base + align * offset[4]), 0x1);
    t2 = _mm256_insertf128_ps(
        _mm256_castps128_ps256(_mm_loadu_ps(base + align * offset[1])),
        _mm_loadu_ps(base + align * offset[5]), 0x1);
    t3 = _mm256_insertf128_ps(
        _mm256_castps128_ps256(_mm_loadu_ps(base + align * offset[2])),
        _mm_loadu_ps(base + align * offset[6]), 0x1);
    t4 = _mm256_insertf128_ps(
        _mm256_castps128_ps256(_mm_loadu_ps(base + align * offset[3])),
  ...
  v0->simdInternal_ =
      nsimd::pack<float>(_mm256_shuffle_ps(t5, t6, _MM_SHUFFLE(1, 0, 1, 0)));
  v1->simdInternal_ =
      nsimd::pack<float>(_mm256_shuffle_ps(t5, t6, _MM_SHUFFLE(3, 2, 3, 2)));
  v2->simdInternal_ =
      nsimd::pack<float>(_mm256_shuffle_ps(t7, t8, _MM_SHUFFLE(1, 0, 1, 0)));
}
...
transposeScatterStoreU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  avx256Transpose3By4InLanes(&v0.simdInternal_, &v1.simdInternal_,
                             &v2.simdInternal_, &tv3);
  _mm_maskstore_ps(base + align * offset[0], mask,
                   _mm256_castps256_ps128(v0.simdInternal_.native_register()));

  _mm_maskstore_ps(base + align * offset[1], mask,
                   _mm256_castps256_ps128(v1.simdInternal_.native_register()));

  _mm_maskstore_ps(base + align * offset[2], mask,
                   _mm256_castps256_ps128(v2.simdInternal_.native_register()));

  _mm_maskstore_ps(base + align * offset[3], mask,
                   _mm256_castps256_ps128(tv3));

  _mm_maskstore_ps(
      base + align * offset[4], mask,
      _mm256_extractf128_ps(v0.simdInternal_.native_register(), 0x1));

  _mm_maskstore_ps(
      base + align * offset[5], mask,
      _mm256_extractf128_ps(v1.simdInternal_.native_register(), 0x1));

  _mm_maskstore_ps(
      base + align * offset[6], mask,
      _mm256_extractf128_ps(v2.simdInternal_.native_register(), 0x1));

  _mm_maskstore_ps(base + align * offset[7], mask,
                   _mm256_extractf128_ps(tv3, 0x1));
}
...
transposeScatterIncrU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  if (align < 4) {
    t5 = _mm256_unpacklo_ps(v1.simdInternal_.native_register(),
                            v2.simdInternal_.native_register());
    t6 = _mm256_unpackhi_ps(v1.simdInternal_.native_register(),
                            v2.simdInternal_.native_register());
    t7 = _mm256_shuffle_ps(v0.simdInternal_.native_register(), t5,
                           _MM_SHUFFLE(1, 0, 0, 0));
    t8 = _mm256_shuffle_ps(v0.simdInternal_.native_register(), t5,
                           _MM_SHUFFLE(3, 2, 0, 1));
    t9 = _mm256_shuffle_ps(v0.simdInternal_.native_register(), t6,
                           _MM_SHUFFLE(1, 0, 0, 2));
    t10 = _mm256_shuffle_ps(v0.simdInternal_.native_register(), t6,
                            _MM_SHUFFLE(3, 2, 0, 3));
    ...
    tX = _mm_loadh_pi(tX,
                      reinterpret_cast<__m64 *>(base + align * offset[4] + 1));
    tX = _mm_add_ps(tX, tE);
    ...
    tX = _mm_loadh_pi(tX,
                      reinterpret_cast<__m64 *>(base + align * offset[5] + 1));
    tX = _mm_add_ps(tX, tF);
    ...
    tX = _mm_loadh_pi(tX,
                      reinterpret_cast<__m64 *>(base + align * offset[6] + 1));
    tX = _mm_add_ps(tX, tG);
    ...
    tX = _mm_loadh_pi(tX,
                      reinterpret_cast<__m64 *>(base + align * offset[7] + 1));
    tX = _mm_add_ps(tX, tH);
    _mm_store_ss(base + align * offset[7], tX);
    _mm_storeh_pi(reinterpret_cast<__m64 *>(base + align * offset[7] + 1), tX);
  } else {
    // Extra elements means we can use full width-4 load/store operations
    t1 = _mm256_unpacklo_ps(v0.simdInternal_.native_register(),
                            v2.simdInternal_.native_register());
    t2 = _mm256_unpackhi_ps(v0.simdInternal_.native_register(),
                            v2.simdInternal_.native_register());
    t3 = _mm256_unpacklo_ps(v1.simdInternal_.native_register(),
                            _mm256_setzero_ps());
    t4 = _mm256_unpackhi_ps(v1.simdInternal_.native_register(),
                            _mm256_setzero_ps());
    ...
    if (align % 4 == 0) {
      ...
      _mm_store_ps(base + align * offset[0],
                   _mm_add_ps(_mm_load_ps(base + align * offset[0]),
                              _mm256_castps256_ps128(t5)));
      _mm_store_ps(base + align * offset[1],
                   _mm_add_ps(_mm_load_ps(base + align * offset[1]),
                              _mm256_castps256_ps128(t6)));
      _mm_store_ps(base + align * offset[2],
                   _mm_add_ps(_mm_load_ps(base + align * offset[2]),
                              _mm256_castps256_ps128(t7)));
      _mm_store_ps(base + align * offset[3],
                   _mm_add_ps(_mm_load_ps(base + align * offset[3]),
                              _mm256_castps256_ps128(t8)));
      _mm_store_ps(base + align * offset[4],
                   _mm_add_ps(_mm_load_ps(base + align * offset[4]),
                              _mm256_extractf128_ps(t5, 0x1)));
      _mm_store_ps(base + align * offset[5],
                   _mm_add_ps(_mm_load_ps(base + align * offset[5]),
                              _mm256_extractf128_ps(t6, 0x1)));
      _mm_store_ps(base + align * offset[6],
                   _mm_add_ps(_mm_load_ps(base + align * offset[6]),
                              _mm256_extractf128_ps(t7, 0x1)));
      _mm_store_ps(base + align * offset[7],
                   _mm_add_ps(_mm_load_ps(base + align * offset[7]),
                              _mm256_extractf128_ps(t8, 0x1)));
    } else {
      // alignment >=5, but not a multiple of 4
      _mm_storeu_ps(base + align * offset[0],
                    _mm_add_ps(_mm_loadu_ps(base + align * offset[0]),
                               _mm256_castps256_ps128(t5)));
      _mm_storeu_ps(base + align * offset[1],
                    _mm_add_ps(_mm_loadu_ps(base + align * offset[1]),
                               _mm256_castps256_ps128(t6)));
      _mm_storeu_ps(base + align * offset[2],
                    _mm_add_ps(_mm_loadu_ps(base + align * offset[2]),
                               _mm256_castps256_ps128(t7)));
      _mm_storeu_ps(base + align * offset[3],
                    _mm_add_ps(_mm_loadu_ps(base + align * offset[3]),
                               _mm256_castps256_ps128(t8)));
      _mm_storeu_ps(base + align * offset[4],
                    _mm_add_ps(_mm_loadu_ps(base + align * offset[4]),
                               _mm256_extractf128_ps(t5, 0x1)));
      _mm_storeu_ps(base + align * offset[5],
                    _mm_add_ps(_mm_loadu_ps(base + align * offset[5]),
                               _mm256_extractf128_ps(t6, 0x1)));
      _mm_storeu_ps(base + align * offset[6],
                    _mm_add_ps(_mm_loadu_ps(base + align * offset[6]),
                               _mm256_extractf128_ps(t7, 0x1)));
      _mm_storeu_ps(base + align * offset[7],
                    _mm_add_ps(_mm_loadu_ps(base + align * offset[7]),
                               _mm256_extractf128_ps(t8, 0x1)));
    }
  }
}
...
transposeScatterDecrU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  if (align < 4) {
    ...
    if (align % 4 == 0) {
      // We can use aligned load & store
      _mm_store_ps(base + align * offset[0],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[0]),
                              _mm256_castps256_ps128(t5)));
      _mm_store_ps(base + align * offset[1],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[1]),
                              _mm256_castps256_ps128(t6)));
      _mm_store_ps(base + align * offset[2],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[2]),
                              _mm256_castps256_ps128(t7)));
      _mm_store_ps(base + align * offset[3],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[3]),
                              _mm256_castps256_ps128(t8)));
      _mm_store_ps(base + align * offset[4],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[4]),
                              _mm256_extractf128_ps(t5, 0x1)));
      _mm_store_ps(base + align * offset[5],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[5]),
                              _mm256_extractf128_ps(t6, 0x1)));
      _mm_store_ps(base + align * offset[6],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[6]),
                              _mm256_extractf128_ps(t7, 0x1)));
      _mm_store_ps(base + align * offset[7],
                   _mm_sub_ps(_mm_load_ps(base + align * offset[7]),
                              _mm256_extractf128_ps(t8, 0x1)));
    } else {
      // alignment >=5, but not a multiple of 4
      _mm_storeu_ps(base + align * offset[0],
                    _mm_sub_ps(_mm_loadu_ps(base + align * offset[0]),
                               _mm256_castps256_ps128(t5)));
      _mm_storeu_ps(base + align * offset[1],
                    _mm_sub_ps(_mm_loadu_ps(base + align * offset[1]),
                               _mm256_castps256_ps128(t6)));
      _mm_storeu_ps(base + align * offset[2],
                    _mm_sub_ps(_mm_loadu_ps(base + align * offset[2]),
                               _mm256_castps256_ps128(t7)));
      _mm_storeu_ps(base + align * offset[3],
                    _mm_sub_ps(_mm_loadu_ps(base + align * offset[3]),
                               _mm256_castps256_ps128(t8)));
      _mm_storeu_ps(base + align * offset[4],
                    _mm_sub_ps(_mm_loadu_ps(base + align * offset[4]),
                               _mm256_extractf128_ps(t5, 0x1)));
      _mm_storeu_ps(base + align * offset[5],
                    _mm_sub_ps(_mm_loadu_ps(base + align * offset[5]),
                               _mm256_extractf128_ps(t6, 0x1)));
      _mm_storeu_ps(base + align * offset[6],
                    _mm_sub_ps(_mm_loadu_ps(base + align * offset[6]),
                               _mm256_extractf128_ps(t7, 0x1)));
      _mm_storeu_ps(base + align * offset[7],
                    _mm_sub_ps(_mm_loadu_ps(base + align * offset[7]),
                               _mm256_extractf128_ps(t8, 0x1)));
    }
  }
}
...
static inline void gmx_simdcall expandScalarsToTriplets(SimdFloat scalar,
                                                        ...
                                                        SimdFloat *triplets2) {
  __m256 t0 =
      _mm256_permute2f128_ps(scalar.simdInternal_.native_register(),
                             scalar.simdInternal_.native_register(), 0x21);
  __m256 t1 = _mm256_permute_ps(scalar.simdInternal_.native_register(),
                                _MM_SHUFFLE(1, 0, 0, 0));
  __m256 t2 = _mm256_permute_ps(t0, _MM_SHUFFLE(2, 2, 1, 1));
  __m256 t3 = _mm256_permute_ps(scalar.simdInternal_.native_register(),
                                _MM_SHUFFLE(3, 3, 3, 2));
  triplets0->simdInternal_ = nsimd::pack<float>(_mm256_blend_ps(t1, t2, 0xF0));
  triplets1->simdInternal_ = nsimd::pack<float>(_mm256_blend_ps(t3, t1, 0xF0));
  triplets2->simdInternal_ = nsimd::pack<float>(_mm256_blend_ps(t2, t3, 0xF0));
}

template <int align>
static inline void gmx_simdcall gatherLoadBySimdIntTranspose(
    const float *base, SimdFInt32 simdoffset, SimdFloat *v0, SimdFloat *v1,
    SimdFloat *v2, SimdFloat *v3) {
  alignas(GMX_SIMD_ALIGNMENT) std::int32_t offset[GMX_SIMD_FLOAT_WIDTH];
  _mm256_store_si256(reinterpret_cast<__m256i *>(offset),
                     simdoffset.simdInternal_.native_register());
...
static inline float gmx_simdcall reduceIncr4ReturnSum(float *m, SimdFloat v0,
  ...
  v0.simdInternal_ = _mm256_hadd_ps(v0.simdInternal_.native_register(),
                                    v1.simdInternal_.native_register());
  v2.simdInternal_ = _mm256_hadd_ps(v2.simdInternal_.native_register(),
                                    v3.simdInternal_.native_register());
  v0.simdInternal_ = _mm256_hadd_ps(v0.simdInternal_.native_register(),
                                    v2.simdInternal_.native_register());
  t0 = _mm_add_ps(
...
static inline SimdFloat gmx_simdcall loadDualHsimd(const float *m0,
  ...
  return {nsimd::pack<float>(_mm256_insertf128_ps(
      _mm256_castps128_ps256(_mm_load_ps(m0)), _mm_load_ps(m1), 0x1))};
}

static inline SimdFloat gmx_simdcall loadDuplicateHsimd(const float *m) {
  assert(std::size_t(m) % 16 == 0);

  return {nsimd::pack<float>(
...
static inline void gmx_simdcall storeDualHsimd(float *m0, float *m1,
  ...
  _mm_store_ps(m1,
               _mm256_extractf128_ps(a.simdInternal_.native_register(), 0x1));
}
...
static inline void gmx_simdcall incrDualHsimd(float *m0, float *m1,
  ...
  assert(std::size_t(m1) % 16 == 0);
  _mm_store_ps(
      m0, _mm_add_ps(_mm256_castps256_ps128(a.simdInternal_.native_register()),
                     _mm_load_ps(m0)));
  _mm_store_ps(m1, _mm_add_ps(_mm256_extractf128_ps(
                                  a.simdInternal_.native_register(), 0x1),
                              _mm_load_ps(m1)));
}

static inline void gmx_simdcall decrHsimd(float *m, SimdFloat a) {
  assert(std::size_t(m) % 16 == 0);
  __m128 asum = _mm_add_ps(
      _mm256_castps256_ps128(a.simdInternal_.native_register()),
      _mm256_extractf128_ps(a.simdInternal_.native_register(), 0x1));
...
static inline void gmx_simdcall gatherLoadTransposeHsimd(
  ...
  t0 =
      _mm_loadl_pi(_mm_setzero_ps(),
                   reinterpret_cast<const __m64 *>(base0 + align * offset[0]));
  t1 =
      _mm_loadl_pi(_mm_setzero_ps(),
                   reinterpret_cast<const __m64 *>(base0 + align * offset[1]));
  t2 =
      _mm_loadl_pi(_mm_setzero_ps(),
                   reinterpret_cast<const __m64 *>(base0 + align * offset[2]));
  t3 =
      _mm_loadl_pi(_mm_setzero_ps(),
                   reinterpret_cast<const __m64 *>(base0 + align * offset[3]));
  t4 =
      _mm_loadl_pi(_mm_setzero_ps(),
                   reinterpret_cast<const __m64 *>(base1 + align * offset[0]));
  t5 =
      _mm_loadl_pi(_mm_setzero_ps(),
                   reinterpret_cast<const __m64 *>(base1 + align * offset[1]));
  t6 =
      _mm_loadl_pi(_mm_setzero_ps(),
                   reinterpret_cast<const __m64 *>(base1 + align * offset[2]));
  t7 =
      _mm_loadl_pi(_mm_setzero_ps(),
                   reinterpret_cast<const __m64 *>(base1 + align * offset[3]));
...
static inline float gmx_simdcall reduceIncr4ReturnSumHsimd(float *m,
  ...
  v0.simdInternal_ = nsimd::pack<float>(_mm256_hadd_ps(
      v0.simdInternal_.native_register(), v1.simdInternal_.native_register()));
  t0 = _mm256_extractf128_ps(v0.simdInternal_.native_register(), 0x1);
  t0 = _mm_hadd_ps(_mm256_castps256_ps128(v0.simdInternal_.native_register()),
                   t0);
...
static inline SimdFloat gmx_simdcall loadU4NOffset(const float *m,
                                                   int offset) {
  return {_mm256_insertf128_ps(_mm256_castps128_ps256(_mm_loadu_ps(m)),
                               _mm_loadu_ps(m + offset), 0x1)};
}
...
template <int align>
static inline void gmx_simdcall
gatherLoadUTransposeSafe(const float *base, const std::int32_t offset[],
                         SimdFloat *v0, SimdFloat *v1, SimdFloat *v2) {
  assert(std::size_t(offset) % 32 == 0);
  ...
  *v0 = _mm256_i32gather_ps(base + 0, vindex.simdInternal_.native_register(),
                            sizeof(float));
  *v1 = _mm256_i32gather_ps(base + 1, vindex.simdInternal_.native_register(),
                            sizeof(float));
  *v2 = _mm256_i32gather_ps(base + 2, vindex.simdInternal_.native_register(),
                            sizeof(float));
}
...
template <int n> SimdFInt32 fastMultiply(SimdFInt32 x) {
  if (n == 2) {
...
static inline void gmx_simdcall gatherLoadBySimdIntTranspose(const float *base,
  ...
  v->simdInternal_ = _mm512_i32gather_ps(
      offset.simdInternal_.native_register(), base, sizeof(float) * align_);
...
transposeScatterIncrU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  if (align < 4) {
    t5 = _mm512_unpacklo_ps(v0.simdInternal_.native_register(),
                            v1.simdInternal_.native_register());
    t6 = _mm512_unpackhi_ps(v0.simdInternal_.native_register(),
                            v1.simdInternal_.native_register());
    t[0] = _mm512_shuffle_ps(t5, v2.simdInternal_.native_register(),
                             _MM_SHUFFLE(0, 0, 1, 0));
    t[1] = _mm512_shuffle_ps(t5, v2.simdInternal_.native_register(),
                             _MM_SHUFFLE(1, 1, 3, 2));
    t[2] = _mm512_shuffle_ps(t6, v2.simdInternal_.native_register(),
                             _MM_SHUFFLE(2, 2, 1, 0));
    t[3] = _mm512_shuffle_ps(t6, v2.simdInternal_.native_register(),
                             _MM_SHUFFLE(3, 3, 3, 2));
    for (i = 0; i < 4; i++) {
      _mm512_mask_storeu_ps(
          base + o[i], avx512Int2Mask(7),
      ...
      _mm512_mask_storeu_ps(
          base + o[8 + i], avx512Int2Mask(7),
          _mm512_castps128_ps512(_mm_add_ps(_mm_loadu_ps(base + o[8 + i]),
                                            _mm512_extractf32x4_ps(t[i], 2))));
      _mm512_mask_storeu_ps(
          base + o[12 + i], avx512Int2Mask(7),
          _mm512_castps128_ps512(_mm_add_ps(_mm_loadu_ps(base + o[12 + i]),
                                            _mm512_extractf32x4_ps(t[i], 3))));
    }
  } else {
    // One could use shuffle here too if it is OK to overwrite the padded
    // elements for alignment
    t5 = _mm512_unpacklo_ps(v0.simdInternal_.native_register(),
                            v2.simdInternal_.native_register());
    t6 = _mm512_unpackhi_ps(v0.simdInternal_.native_register(),
                            v2.simdInternal_.native_register());
    t7 = _mm512_unpacklo_ps(v1.simdInternal_.native_register(),
                            _mm512_setzero_ps());
    t8 = _mm512_unpackhi_ps(v1.simdInternal_.native_register(),
                            _mm512_setzero_ps());
    ...
    if (align % 4 == 0) {
      for (i = 0; i < 4; i++) {
        _mm_store_ps(base + o[i], _mm_add_ps(_mm_load_ps(base + o[i]),
                                             _mm512_castps512_ps128(t[i])));
        _mm_store_ps(base + o[4 + i],
                     _mm_add_ps(_mm_load_ps(base + o[4 + i]),
                                _mm512_extractf32x4_ps(t[i], 1)));
        _mm_store_ps(base + o[8 + i],
                     _mm_add_ps(_mm_load_ps(base + o[8 + i]),
                                _mm512_extractf32x4_ps(t[i], 2)));
        _mm_store_ps(base + o[12 + i],
                     _mm_add_ps(_mm_load_ps(base + o[12 + i]),
                                _mm512_extractf32x4_ps(t[i], 3)));
      }
    } else {
      for (i = 0; i < 4; i++) {
        _mm_storeu_ps(base + o[i], _mm_add_ps(_mm_loadu_ps(base + o[i]),
                                              _mm512_castps512_ps128(t[i])));
        _mm_storeu_ps(base + o[4 + i],
                      _mm_add_ps(_mm_loadu_ps(base + o[4 + i]),
                                 _mm512_extractf32x4_ps(t[i], 1)));
        _mm_storeu_ps(base + o[8 + i],
                      _mm_add_ps(_mm_loadu_ps(base + o[8 + i]),
                                 _mm512_extractf32x4_ps(t[i], 2)));
        _mm_storeu_ps(base + o[12 + i],
                      _mm_add_ps(_mm_loadu_ps(base + o[12 + i]),
                                 _mm512_extractf32x4_ps(t[i], 3)));
      }
    }
  }
...
transposeScatterDecrU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  if (align < 4) {
    ...
    if (align % 4 == 0) {
      for (i = 0; i < 4; i++) {
        _mm_store_ps(base + o[i], _mm_sub_ps(_mm_load_ps(base + o[i]),
                                             _mm512_castps512_ps128(t[i])));
        _mm_store_ps(base + o[4 + i],
                     _mm_sub_ps(_mm_load_ps(base + o[4 + i]),
                                _mm512_extractf32x4_ps(t[i], 1)));
        _mm_store_ps(base + o[8 + i],
                     _mm_sub_ps(_mm_load_ps(base + o[8 + i]),
                                _mm512_extractf32x4_ps(t[i], 2)));
        _mm_store_ps(base + o[12 + i],
                     _mm_sub_ps(_mm_load_ps(base + o[12 + i]),
                                _mm512_extractf32x4_ps(t[i], 3)));
      }
    } else {
      for (i = 0; i < 4; i++) {
        _mm_storeu_ps(base + o[i], _mm_sub_ps(_mm_loadu_ps(base + o[i]),
                                              _mm512_castps512_ps128(t[i])));
        _mm_storeu_ps(base + o[4 + i],
                      _mm_sub_ps(_mm_loadu_ps(base + o[4 + i]),
                                 _mm512_extractf32x4_ps(t[i], 1)));
        _mm_storeu_ps(base + o[8 + i],
                      _mm_sub_ps(_mm_loadu_ps(base + o[8 + i]),
                                 _mm512_extractf32x4_ps(t[i], 2)));
        _mm_storeu_ps(base + o[12 + i],
                      _mm_sub_ps(_mm_loadu_ps(base + o[12 + i]),
                                 _mm512_extractf32x4_ps(t[i], 3)));
      }
    }
  }
...
static inline float gmx_simdcall reduceIncr4ReturnSum(float *m, SimdFloat v0,
  ...
  t0 = _mm512_add_ps(
      ...
      _mm512_permute_ps(v0.simdInternal_.native_register(), 0x4E));
  t0 = _mm512_mask_add_ps(
      t0, avx512Int2Mask(0xCCCC), v2.simdInternal_.native_register(),
      _mm512_permute_ps(v2.simdInternal_.native_register(), 0x4E));
  t1 = _mm512_add_ps(
      v1.simdInternal_.native_register(),
      _mm512_permute_ps(v1.simdInternal_.native_register(), 0x4E));
  t1 = _mm512_mask_add_ps(
      t1, avx512Int2Mask(0xCCCC), v3.simdInternal_.native_register(),
      _mm512_permute_ps(v3.simdInternal_.native_register(), 0x4E));
  t2 = _mm512_add_ps(t0, _mm512_permute_ps(t0, 0xB1));
  t2 = _mm512_mask_add_ps(t2, avx512Int2Mask(0xAAAA), t1,
                          _mm512_permute_ps(t1, 0xB1));
...
static inline SimdFloat gmx_simdcall loadDualHsimd(const float *m0,
  ...
  return {_mm512_castpd_ps(_mm512_insertf64x4(
      _mm512_castpd256_pd512(
          _mm256_load_pd(reinterpret_cast<const double *>(m0))),
      _mm256_load_pd(reinterpret_cast<const double *>(m1)), 1))};
}

static inline SimdFloat gmx_simdcall loadDuplicateHsimd(const float *m) {
  assert(std::size_t(m) % 32 == 0);
  return {_mm512_castpd_ps(_mm512_broadcast_f64x4(
      _mm256_load_pd(reinterpret_cast<const double *>(m))))};
}
...
static inline void gmx_simdcall storeDualHsimd(float *m0, float *m1,
  ...
  _mm256_store_ps(m0,
                  _mm512_castps512_ps256(a.simdInternal_.native_register()));
...
static inline void gmx_simdcall incrDualHsimd(float *m0, float *m1,
  ...
  x = _mm256_load_ps(m1);
  x = _mm256_add_ps(
      x, _mm256_castpd_ps(_mm512_extractf64x4_pd(
             _mm512_castps_pd(a.simdInternal_.native_register()), 1)));
...
static inline void gmx_simdcall decrHsimd(float *m, SimdFloat a) {
  ...
  a.simdInternal_ = _mm512_add_ps(
      a.simdInternal_.native_register(),
      _mm512_shuffle_f32x4(a.simdInternal_.native_register(),
                           a.simdInternal_.native_register(), 0xEE));
  t = _mm256_load_ps(m);
  t = _mm256_sub_ps(t,
                    _mm512_castps512_ps256(a.simdInternal_.native_register()));
  _mm256_store_ps(m, t);
...
static inline void gmx_simdcall gatherLoadTransposeHsimd(
  ...
  static_assert(align == 2 || align == 4,
                "If more are needed use fastMultiply");
  if (align == 4) {
    idx = _mm256_slli_epi32(idx, 1);
  }

  tmp1 = _mm512_castpd_ps(_mm512_i32gather_pd(
      idx, reinterpret_cast<const double *>(base0), sizeof(double)));
  tmp2 = _mm512_castpd_ps(_mm512_i32gather_pd(
      idx, reinterpret_cast<const double *>(base1), sizeof(double)));
...
static inline float gmx_simdcall reduceIncr4ReturnSumHsimd(float *m,
  ...
  t0 = _mm512_shuffle_f32x4(v0.simdInternal_.native_register(),
                            v1.simdInternal_.native_register(), 0x88);
  t1 = _mm512_shuffle_f32x4(v0.simdInternal_.native_register(),
                            v1.simdInternal_.native_register(), 0xDD);
...
static inline SimdFloat gmx_simdcall loadU4NOffset(const float *f,
  ...
  const __m256i gdx =
      _mm256_add_epi32(_mm256_setr_epi32(0, 2, 0, 2, 0, 2, 0, 2),
                       _mm256_mullo_epi32(idx, _mm256_set1_epi32(offset)));
  return {_mm512_castpd_ps(_mm512_i32gather_pd(
      gdx, reinterpret_cast<const double *>(f), sizeof(float)))};
}
...
static inline void gmx_simdcall gatherLoadTranspose(
  ...
  float32x4x2_t t0 = vuzpq_f32(vld1q_f32(base + align * offset[0]),
                               vld1q_f32(base + align * offset[2]));
  float32x4x2_t t1 = vuzpq_f32(vld1q_f32(base + align * offset[1]),
                               vld1q_f32(base + align * offset[3]));
...
gatherLoadTranspose(const float *base, const std::int32_t offset[],
  ...
  float32x4x2_t tmp = vtrnq_f32(v0->simdInternal_.native_register(),
                                v1->simdInternal_.native_register());
...
transposeScatterStoreU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  vst1q_lane_f32(base + align * offset[0] + 2,
                 v2.simdInternal_.native_register(), 0);
  vst1q_lane_f32(base + align * offset[1] + 2,
                 v2.simdInternal_.native_register(), 1);
  vst1q_lane_f32(base + align * offset[2] + 2,
                 v2.simdInternal_.native_register(), 2);
  vst1q_lane_f32(base + align * offset[3] + 2,
                 v2.simdInternal_.native_register(), 3);
}
...
transposeScatterIncrU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  if (align < 4) {
    ...
    base[align * offset[0] + 2] +=
        vgetq_lane_f32(v2.simdInternal_.native_register(), 0);

    t1 = vadd_f32(t1, vld1_f32(base + align * offset[1]));
    vst1_f32(base + align * offset[1], t1);
    base[align * offset[1] + 2] +=
        vgetq_lane_f32(v2.simdInternal_.native_register(), 1);

    t2 = vadd_f32(t2, vld1_f32(base + align * offset[2]));
    vst1_f32(base + align * offset[2], t2);
    base[align * offset[2] + 2] +=
        vgetq_lane_f32(v2.simdInternal_.native_register(), 2);

    t3 = vadd_f32(t3, vld1_f32(base + align * offset[3]));
    vst1_f32(base + align * offset[3], t3);
    base[align * offset[3] + 2] +=
        vgetq_lane_f32(v2.simdInternal_.native_register(), 3);
  } else {
    // Extra elements means we can use full width-4 load/store operations
    float32x4x2_t t0 = vuzpq_f32(v0.simdInternal_.native_register(),
                                 v2.simdInternal_.native_register());
    float32x4x2_t t1 =
        vuzpq_f32(v1.simdInternal_.native_register(), vdupq_n_f32(0.0f));
    float32x4x2_t t2 = vtrnq_f32(t0.val[0], t1.val[0]);
    float32x4x2_t t3 = vtrnq_f32(t0.val[1], t1.val[1]);
    ...
    vst1q_f32(base + align * offset[0],
              vaddq_f32(t4, vld1q_f32(base + align * offset[0])));
    vst1q_f32(base + align * offset[1],
              vaddq_f32(t5, vld1q_f32(base + align * offset[1])));
    vst1q_f32(base + align * offset[2],
              vaddq_f32(t6, vld1q_f32(base + align * offset[2])));
    vst1q_f32(base + align * offset[3],
              vaddq_f32(t7, vld1q_f32(base + align * offset[3])));
  }
}
...
transposeScatterDecrU(float *base, const std::int32_t offset[], SimdFloat v0,
  ...
  if (align < 4) {
    ...
    vst1q_f32(base + align * offset[0],
              vsubq_f32(vld1q_f32(base + align * offset[0]), t4));
    vst1q_f32(base + align * offset[1],
              vsubq_f32(vld1q_f32(base + align * offset[1]), t5));
    vst1q_f32(base + align * offset[2],
              vsubq_f32(vld1q_f32(base + align * offset[2]), t6));
    vst1q_f32(base + align * offset[3],
              vsubq_f32(vld1q_f32(base + align * offset[3]), t7));
  }
}
