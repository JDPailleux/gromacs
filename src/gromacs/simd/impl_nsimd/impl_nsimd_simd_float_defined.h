...
static inline SimdFloat gmx_simdcall frexp(SimdFloat value,
  ...
  const __m128i exponentBias =
      _mm_set1_epi32(126); // add 1 to make our definition identical to frexp()
  const __m128 half = _mm_set1_ps(0.5f);
  __m128i iExponent;

  iExponent = _mm_castps_si128(
      _mm_and_ps(value.simdInternal_.native_register(), exponentMask));
  iExponent = _mm_sub_epi32(_mm_srli_epi32(iExponent, 23), exponentBias);
  exponent->simdInternal_ = iExponent;

  return {_mm_or_ps(
      _mm_and_ps(value.simdInternal_.native_register(), mantissaMask), half)};
}

static inline float gmx_simdcall reduce(SimdFloat a) {
  // Shuffle has latency 1/throughput 1, followed by add with latency 3,
  // t-put 1. This is likely faster than using _mm_hadd_ps, which has latency
  // 5, t-put 2.
  a.simdInternal_ =
      _mm_add_ps(a.simdInternal_.native_register(),
                 _mm_shuffle_ps(a.simdInternal_.native_register(),
                                a.simdInternal_.native_register(),
                                _MM_SHUFFLE(1, 0, 3, 2)));
  a.simdInternal_ =
      _mm_add_ss(a.simdInternal_.native_register(),
                 _mm_shuffle_ps(a.simdInternal_.native_register(),
                                a.simdInternal_.native_register(),
                                _MM_SHUFFLE(0, 3, 2, 1)));
...
static inline SimdFInt32 gmx_simdcall cvttR2I(SimdFloat a) {
  return {_mm_cvttps_epi32(a.simdInternal_.native_register())};
}
...
static inline SimdFloat gmx_simdcall ldexp(SimdFloat value,
  ...
  return {_mm_mul_ps(value.simdInternal_.native_register(),
...
static inline SimdFloat gmx_simdcall frexp(SimdFloat value,
                                           SimdFInt32 *exponent) {
  const __m256 exponentMask =
      _mm256_castsi256_ps(_mm256_set1_epi32(0x7F800000));
  const __m256 mantissaMask =
      _mm256_castsi256_ps(_mm256_set1_epi32(0x807FFFFF));
  const __m256i exponentBias = _mm256_set1_epi32(
      126); // add 1 to make our definition identical to frexp()
  const __m256 half = _mm256_set1_ps(0.5);
  __m256i iExponent;

  iExponent =
      _mm256_castps_si256(_mm256_and_ps(value.simdInternal_, exponentMask));
  exponent->simdInternal_ =
      _mm256_sub_epi32(_mm256_srli_epi32(iExponent, 23), exponentBias);
...
static inline SimdFloat gmx_simdcall frexp(SimdFloat value,
  ...
  iExponent = _mm256_castps_si256(
      _mm256_and_ps(value.simdInternal_.native_register(), exponentMask));
  ...
  exponent->simdInternal_ =
      _mm256_insertf128_si256(iExponent, iExponentHigh, 0x1);

  return {_mm256_or_ps(
      _mm256_and_ps(value.simdInternal_.native_register(), mantissaMask),
      half)};
}
...
static inline SimdFloat gmx_simdcall ldexp(SimdFloat value,
  ...
  iExponentHigh =
      _mm256_extractf128_si256(exponent.simdInternal_.native_register(), 0x1);
  iExponentLow =
      _mm256_castsi256_si128(exponent.simdInternal_.native_register());
  ...
  return {_mm256_mul_ps(value.simdInternal_.native_register(),
...
static inline SimdFInt32 gmx_simdcall cvttR2I(SimdFloat a) {
  return {_mm256_cvttps_epi32(a.simdInternal_.native_register())};
}

template <int index>
static inline std::int32_t gmx_simdcall extract(SimdFInt32 a) {
  return _mm_extract_epi32(
      _mm256_extractf128_si256(a.simdInternal_.native_register(), index >> 2),
      index & 0x3);
}
...
static inline SimdFloat gmx_simdcall frexp(SimdFloat value,
  ...
  return {_mm512_getmant_ps(value.simdInternal_.native_register(),
...
static inline SimdFloat gmx_simdcall ldexp(SimdFloat value,
  ...
  __m512i iExponent =
      _mm512_add_epi32(exponent.simdInternal_.native_register(), exponentBias);
  ...
  return {_mm512_mul_ps(value.simdInternal_.native_register(),
...
static inline SimdFInt32 gmx_simdcall cvttR2I(SimdFloat a) {
  return {_mm512_cvttps_epi32(a.simdInternal_.native_register())};
}
...
static inline SimdFloat gmx_simdcall frexp(SimdFloat value,
  ...
  const int32x4_t exponentBias =
      vdupq_n_s32(126); // add 1 to make our definition identical to frexp()
  const float32x4_t half = vdupq_n_f32(0.5f);
  int32x4_t iExponent;

  iExponent =
      vandq_s32(vreinterpretq_s32_f32(value.simdInternal_.native_register()),
                exponentMask);
...
static inline SimdFloat gmx_simdcall ldexp(SimdFloat value,
  ...
  int32x4_t iExponent =
      vaddq_s32(exponent.simdInternal_.native_register(), exponentBias);
  ...
  return {vmulq_f32(value.simdInternal_.native_register(),
...
static inline SimdFloat gmx_simdcall rsqrtIter(SimdFloat lu, SimdFloat x) {
  return {vmulq_f32(lu.simdInternal_.native_register(),
                    vrsqrtsq_f32(vmulq_f32(lu.simdInternal_.native_register(),
...
static inline SimdFloat gmx_simdcall rcpIter(SimdFloat lu, SimdFloat x) {
  return {vmulq_f32(lu.simdInternal_.native_register(),
                    vrecpsq_f32(lu.simdInternal_.native_register(),
                                x.simdInternal_.native_register()))};
}
