...
avx256Transpose3By4InLanes(nsimd::pack<float> * v0,
                           ...
                           nsimd::pack<float> * v3)
{
    nsimd::pack<float> t1 = _mm256_unpacklo_ps(*v0, *v1);
    nsimd::pack<float> t2 = _mm256_unpackhi_ps(*v0, *v1);
                    ...
                    SimdFloat *          v3)
{
    ...
    t1 = nsimd::loada<nsimd::pack<float> >(base + align * offset[0]);
    t2 = nsimd::loada<nsimd::pack<float> >(base + align * offset[1]);
    t3 = nsimd::loada<nsimd::pack<float> >(base + align * offset[2]);
    t4 = nsimd::loada<nsimd::pack<float> >(base + align * offset[3]);
    t5 = nsimd::loada<nsimd::pack<float> >(base + align * offset[4]);
    t6 = nsimd::loada<nsimd::pack<float> >(base + align * offset[5]);
    t7 = nsimd::loada<nsimd::pack<float> >(base + align * offset[6]);
    t8 = nsimd::loada<nsimd::pack<float> >(base + align * offset[7]);
                      ...
                      SimdFloat            v2)
{
    ...
    else
    {
        ...
        t3  = _mm256_unpacklo_ps(v1.simdInternal_, nsimd::set1<nsimd::pack<float> >(0));
        t4  = _mm256_unpackhi_ps(v1.simdInternal_, nsimd::set1<nsimd::pack<float> >(0));
        t5  = _mm256_unpacklo_ps(t1, t3);                             // x0 y0 z0  0 | x4 y4 z4 
        t6  = _mm256_unpackhi_ps(t1, t3);                             // x1 y1 z1  0 | x5 y5 z5 
        t7  = _mm256_unpacklo_ps(t2, t4);                             // x2 y2 z2  0 | x6 y6 z6 
...
#endif // GMX_SIMD_IMPL_X86_AVX_256_UTIL_FLOAT_
