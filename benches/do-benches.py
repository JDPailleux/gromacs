...
def benchmark(simd, nsimd_path):
  ...
  if simd != "nsimd":
    ...
    os.system("cd ../build/; cmake .. -DGMX_SIMD=NSIMD -DGMX_MPI=on -DGMX_NSIMD_PATH=" + str(nsimd_path) +" -DGMX_NSIMD_FOR="+ str(simd) +" -DCMAKE_PREFIX_PATH=" + nsimd_path +"/build")
    os.system("cd ../build/; make -j30")
    os.system("cd ../build/; cmake .. -DGMX_SIMD=NSIMD -DGMX_MPI=off -DGMX_NSIMD_PATH=" + str(nsimd_path) +" -DGMX_NSIMD_FOR="+ str(simd) +" -DCMAKE_PREFIX_PATH=" + nsimd_path +"/build")
    os.system("cd ../build/; make -j30")
    os.system(cmd_bench)
